...
namespace Cake.Host.Scripting.Roslyn.Nightly
{
    using Cake.Core.IO;

    public sealed class RoslynNightlyScriptSessionFactory
    {
        ...
        private void Install(SemanticVersion version)
        {
            ...
            var installRoot = root.MakeAbsolute(_environment).Combine(Guid.NewGuid().ToString("N", CultureInfo.InvariantCulture));
